...
//original
bool Turtlebot3MotorDriver::sptcontrolMotor(const float wheel_separation, float* value)
{
  bool dxl_comm_result = false;

  float wheel_velocity_spt[2];

  wheel_velocity_spt[LEFT]  = value[LEFT];
  wheel_velocity_spt[RIGHT]  = value[RIGHT];

  wheel_velocity_spt[LEFT]  = constrain(wheel_velocity_spt[LEFT]  * VELOCITY_CONSTANT_VALUE, -LIMIT_X_MAX_VELOCITY, LIMIT_X_MAX_VELOCITY);
  wheel_velocity_spt[RIGHT] = constrain(wheel_velocity_spt[RIGHT] * VELOCITY_CONSTANT_VALUE, -LIMIT_X_MAX_VELOCITY, LIMIT_X_MAX_VELOCITY);

  dxl_comm_result = writeVelocity((int64_t)wheel_velocity_spt[LEFT], (int64_t)wheel_velocity_spt[RIGHT]);
  if (dxl_comm_result == false)
    return false;
